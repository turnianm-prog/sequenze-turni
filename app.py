...
st.write(
    ...
    " autonomamente i dati reali, filtrerà solo i cambi validi e genererà i"
    " PDF."
)

# Configurazione API Key (puoi inserire la tua chiave o configurarla nei secrets di Streamlit)
# api_key = st.secrets.get("GEMINI_API_KEY", "INSERISCI_QUI_LA_TUA_API_KEY")
api_key = st.text_input(
    "Inserisci la tua Google Gemini API Key:", type="password"
)
...
if uploaded_file is not None and api_key:
  ...
  if st.button("🚀 Estrai Dati con IA e Genera i 3 PDF", type="primary"):
    with st.spinner(
      ...
      try:
        ...
        # Prompt strutturato per estrarre solo i dati reali validi
        prompt = (
            "Analizza questa immagine di una tabella di cambi turno ferroviari."
            " Estrai SOLO le righe in cui la terza colonna (Assegnato A) è"
            " valida (ignora i riposi 'R', 'NO', i trattini '-' o i campi"
            " vuoti). Restituisci i dati in formato strutturato separato da"
            " punto e virgola per ogni riga valida in questo esatto formato:"
            " COGNOME;CODICE;TURNO;ASSEGNATO_A. Includi anche eventuali"
            " annotazioni manoscritte in fondo alla pagina se presenti."
        )
        ...
        if not data_giornaliera:
          st.error(
              ...
              " nitidezza della foto o l'API Key."
          )
        else:
  ...
  st.warning("⚠️ Inserisci la tua API Key per procedere con la lettura della foto.")
